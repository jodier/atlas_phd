...
\begin{figure}[!p]
  \begin{center}
    \includegraphics[width=1.0\linewidth, keepaspectratio]{phd_cosmics/vp1_run91890_evt3494207_2048x1604}
  \end{center}
  \caption{Visualisation d'un événement cosmic (run 91890) avec le logiciel Virtual Point 1 (VP1). La ligne rouge correspond à la trace combinée reconstruite du muon.}
\end{figure}
...
\begin{figure}[!h]
  ...
  \end{tabular}
  \end{center}
  \caption{\label{cosmics_s2endcap}Énergie en dehors du c\oe ur de la gerbe $ F_{side} $ pour $ 0<|\phi|<2\pi $ (a).  Largeur totale de la gerbe électromagnétique $ w_{stot} $ pour $ 0<|\phi|<2\pi $ (b).}
\end{figure}
...
Dans ce chapitre, nous avons étudier les objets calorimétriques, électrons d'ionisation et photons de radiation, issus de muons cosmiques. Dans ce but, nous avons défini une sélection permettant de disposer d'une accéptance équivalente pour les données et pour la simulation Monte-Carlo. Nous avons ensuite réalisée la première analyse avec des données réelles qui a permit d'étudier les variables de forme de gerbes électromagnétiques et qui a permit de valider les simulations Monte-Carlo. En dépit de la faible statistiques disponible, nous pouvons conclure que la simulation des variables de forme de gerbes électromagnétiques est réaliste. Nous avons pu observer l'effet de la projectivité ainsi que l'effet de l'amélioration de la simulation de la diaphonie entre les cellules.
