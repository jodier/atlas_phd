...
Dans ce chapitre, nous avons étudié les objets calorimétriques, électrons d'ionisation et photons de radiation, issus de muons cosmiques. Dans ce but, nous avons défini une sélection permettant de disposer d'une accéptance équivalente pour les données et pour la simulation Monte-Carlo. Nous avons ensuite réalisé la première analyse, avec des données réelles, des variables de forme de gerbes électromagnétiques. En dépit de la faible statistiques disponible, nous pouvons conclure que la simulation des variables de forme de gerbes électromagnétiques est réaliste. Finalement, nous avons pu observer l'effet de la projectivité ainsi que l'effet de l'amélioration de la simulation de la diaphonie entre les cellules des deux premiers compartiments.

Cette étude a fourni la première validation des variables de forme de gerbes électromagnétiques. Celles-ci ont de nouveau été étudiées avec les données 2010 à $ \sqrt{s}=7\TeV $\cite{}. La \ref{data2010_shower} montre l'étalement latéral de la gerbe électromagnétique dans la direction $ \eta $ dans le second compartiment : rapport de l'énergie dans un amas $ \Delta\eta\times\Delta\phi=3\times7 $ par l'énergie dans un amas $ \Delta\eta\times\Delta\phi=7\times7 $ (a) et le second moment de la pseudo-rapidité des cellules d'un amas $ \Delta\eta\times\Delta\phi=3\times5 $ pondérées par leur énergie (b). Nous pouvons pouvons observé un désaccord entre les données et la simulation Monte-Carlo, les largeurs et les moyennes sont décalées. Pour les données cosmiques, la distribution $ R_\eta $ (figure \ref{cosmics_reta}) ne présentait pas de désaccord significatif, sans doute à cause de la faible statistique. La distribution $ W_{\eta2} $ (figure \ref{cosmics_weta2}) présentait déjà le décalage observé avec les données.
...
Remarque : Les critères de sélection des électrons présentés au chapite \ref{chapitre_reco} ont été ajustés afin de tenir compte des différences observées entre les données et la simulation Monte-Carlo.
