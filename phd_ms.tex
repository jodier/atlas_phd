...
Il est évident que les doublets de fermions gauches ont une hypercharge $ Y=-1 $. Le modèle standard de Glashow, Salam et Weinberg (modèle GSW) est basé sur le groupe de jauge $ G $ formé du produit direct du groupe $ SU(2) $ pour l'isospin faible et $ U(1) $ pour l'hypercharge :
\begin{equation}
  G=SU(2)_L\times U(1)_Y
\end{equation}

En posant $ g $, $ g' $ et $ Y_j $, des constantes de couplage et $ \sigma^a $, les matrices de Pauli, il est alors possible d'écrire le lagrangien de l'interaction électrofaible :
\begin{eqnarray}\label{lagrangien_electrofaible}
  \mathcal{L}_\mathrm{GSW} & = & \sum^3_{j=1}i\bar\Psi_j\slashed D\Psi^j-\frac{1}{4}W^a_{\mu\nu}W^{\mu\nu}_a-\frac{1}{4}B_{\mu\nu}B^{\mu\nu}\nonumber\\
                           & = & \left[ \sum^3_{j=1}i\bar\Psi_j\slashed\partial\Psi^j-g\bar\Psi_j\frac{\sigma^a}{2}\slashed W_a\Psi^j-g'\bar\Psi_jY_j\slashed B\Psi^j\right]-\frac{1}{4}W^a_{\mu\nu}W^{\mu\nu}_a-\frac{1}{4}B_{\mu\nu}B^{\mu\nu}
\end{eqnarray}
...
Il est possible de faire un choix de jauge pour choisir une configuration particulière du champ $ \Phi_L $ (astuce diabolique !) :
\begin{equation}
  \boxed{\Phi'_L\rightarrow\Phi_{0,L}=\frac{1}{\sqrt{2}}\binom{0}{v+h(x)}}
\end{equation}
...
La masse du boson de Higgs est un \underline{paramètre libre} du modèle standard de la physique de la physique des particules. Il reste maintenant à déterminer comment font les fermions pour acquérir une masse. Examinons le couplage de Yakawa entre le doublets des leptons et le champ $ \Phi_{0,L} $  dans la limite où $ h(x) \rightarrow 0 $ :
\begin{eqnarray}
...
\begin{table}[!htb]
  \begin{center}
    ...
    \begin{tabular}{|l|c|c|c|c|}
      ...
      \hline
      \hline
      Boson(s) & 8 gluons & 1 photon & $ Z $ et $ W^\pm $ & graviton(s) \\
      \hline
    ...
    \begin{fmffile}{higgs_graph1}
      \fmfframe(1,7)(1,7){ 
       \begin{fmfgraph*}(110,62)
        ...
        \fmflabel{$H$}{o1}
        \fmflabel{$H$}{o2}
        \fmflabel{$\ \propto\left\{\lambda=\frac{M^2_\mathrm{Higgs}}{2v^2}\right\}$}{v1}
       \end{fmfgraph*}
      }
    \end{fmffile}
...
Selon Landau, Abrikosov et Khalatnikov, la relation entre la charge observable $ g(\Lambda) $ et la charge nue $ g(\Lambda_0) $, pour les théories de champs renormalisables, est donnée par l'expression (voir annexe ???) :
\begin{equation}
...
où $ \lambda_t $ est le couplage de Yukawa du quart top.
...
Pour que $ \lambda(\Lambda) $ reste positif, la masse du boson de Higgs soit satisfaire à :
\begin{equation}
  \boxed{M^2_\mathrm{Higgs}>\frac{v^2}{4\pi^2}\left[3\lambda^4_y-\frac{3}{16}\left(2g^4+\left(g^2+g'^2\right)^2\right)\right]\frac{\Lambda^2_\mathrm{cut}}{v^2}}
\end{equation}
