...
En utilisant l'expression du projecteur vers la composante de chiralité left (équation \ref{projecteurs}), le lagrangien se réécrit sous sa forme définitive :
\begin{equation}
...
En utilisant l'expression du projecteur vers la composante de chiralité left (équation \ref{projecteurs}), et après développement, le lagrangien $ \mathcal{L}^Z_\mathrm{CN} $ se réécrit sous sa forme définitive :
\begin{eqnarray}
...
Les couplages élémentaires entre un doublet de fermions left et un boson de jauge sont les suivants :
\begin{figure}[!htb]
  \begin{center}
...
Remarque : Dans le modèle standard de la physique de particule, comme les neutrinos n'interagissent que via l'interaction faible alors leur composante right n'existe pas.
...
Dans le cadre d'une théorie quantique des champs, nous avons vu que pour assurer l'invariance de jauge d'un lagrangien, les bosons de jauge doivent être nécessairement de masse nulle. C'est le cas, par exemple, du photon pour l'interaction électromagnétique et des gluons pour l'interaction forte.

En 1983, après la découverte des bosons $ W^\pm $ et $ Z $ au Cern, il fut confirmé de façon indiscutable que ces trois bosons possèdent une masse importante. Actuellement, la masse mesurée du $ W^\pm $ est $ M_W=80.403(29)\GeV $ et la masse mesurée du $ Z $ est $ M_Z=91.1876(21)\GeV $.

Le premier modèle expliquant l'acquisition de masse par les particules fut proposé en 1964 parallèlement par Gerry Guralnik, Carl Richard Hagen, et Tom Kibble, Robert Brout et François Englert ainsi que par Peter Higgs (figure \ref{photographie}). Ce modèle est désormais connu sous le nom de \emph{mécanisme de Higgs}. Il est un pilier du modèle standard de la physique des particules, cependant, sa principale prédiction, le \emph{boson de Higgs}, n'a toujours pas été observé.
...
\begin{itemize}
  ...
  \item $ \mu^2<0 $ $ \Rightarrow $ $ \left\{\begin{array}{l} \mathrm{un\ minimum\ trivial\ }\left<0\right|\Phi_L\left|0\right>=0 \\ \mathrm{un\ minimum\ non\ trivial\ }\boxed{\left<0\right|\Phi_L\left|0\right>=-\frac{\mu^2}{\lambda}\equiv v^2} \end{array}\right. $
\end{itemize}
...
En utilisant les expressions ??? et ???, on obtient :
\begin{displaymath}
...
La masse du boson de Higgs est un \underline{paramètre libre} du modèle standard de la physique de la physique des particules. Il reste maintenant à déterminer comment les fermions acquièrent leur masse. Examinons le couplage de Yakawa entre le doublets des leptons et le champ $ \Phi_{0,L} $  dans la limite où $ h(x) \rightarrow 0 $ :
\begin{eqnarray}
...
où les coefficients $ \lambda_l $ et $ \lambda_q $ sont appelés les couplages de Yukawa. Au final, le modèle de Higgs permet d'expliquer l'origine des masses des bosons $ W^\pm $ et $ Z $ ainsi que celles de tous les fermions. Dans le modèle standard minimal de la physique des particules, les neutrinos ont une masse nulle.
...
\begin{figure}[!htb]
    ...
    \begin{fmffile}{higgs_graph2}
      \fmfframe(1,7)(1,7){ 
       \begin{fmfgraph*}(110,62)
        ...
        \fmflabel{$Z$}{i2}
        \fmflabel{$H$}{o1}
        \fmflabel{$i\frac{g}{\cos\theta_W}M_Zg_{\mu\nu}\ $}{v1}
       \end{fmfgraph*}
      }
    \end{fmffile}
    ...
    \begin{fmffile}{higgs_graph3}
      \fmfframe(1,7)(1,7){ 
       \begin{fmfgraph*}(110,62)
        ...
        \fmflabel{$\bar f$}{i2}
        \fmflabel{$H$}{o1}
        \fmflabel{$-i\frac{g}{2}\frac{m_f}{M_W}$}{v1}
       \end{fmfgraph*}
      }
    \end{fmffile}
    ...
    \begin{fmffile}{higgs_graph4}
      \fmfframe(1,7)(1,7){ 
       \begin{fmfgraph*}(110,62)
        ...
        \fmflabel{$H$}{i2}
        \fmflabel{$H$}{o1}
        \fmflabel{$-i\frac{3g}{2}\frac{M_\mathrm{Higgs}}{M_W}\ $}{v1}
       \end{fmfgraph*}
      }
    \end{fmffile}
  ...
  \end{center}
  \caption{Couplages trilinéaires du boson de Higgs avec les bosons $ W^\pm $ et $ Z $, (a) et (b), avec les fermions (couplages de Yukawa), (c) et auto-couplage (d).}
\end{figure}
...
\begin{figure}[!htb]
    ...
    \begin{fmffile}{higgs_graph6}
      \fmfframe(1,7)(1,7){ 
       \begin{fmfgraph*}(110,62)
        ...
        \fmflabel{$H$}{o1}
        \fmflabel{$H$}{o2}
        \fmflabel{$\ i\frac{g^2}{2\cos^2\theta_W}g_{\mu\nu}$}{v1}
       \end{fmfgraph*}
      }
    \end{fmffile}
    ...
    \begin{fmffile}{higgs_graph7}
      \fmfframe(1,7)(1,7){ 
       \begin{fmfgraph*}(110,62)
        ...
        \fmflabel{$H$}{o1}
        \fmflabel{$H$}{o2}
        \fmflabel{$\ -i\frac{3g^2}{4}\frac{M^2_\mathrm{Higgs}}{M^2_W}$}{v1}
       \end{fmfgraph*}
      }
    \end{fmffile}
  ...
  \end{center}
  \caption{Couplages quadrilinéaires du boson de Higgs avec les bosons $ W^\pm $ et $ Z $, (a) et (b) et auto-couplage, (c).}
\end{figure}
...
\begin{table}[!htb]
  \begin{center}
    ...
    \begin{tabular}{|l|c|c|c|c|}
      ...
      Masse & 0 & 0 & & 0 \\
      \hline
      Portée & $ \infty $, confinée $ <10^{-15} $ m & $ \infty $ & $ <10^{-18} $ m & $ \infty $ \\
      \hline
...
\begin{table}[!htb]
  \begin{center}
    ...
    \begin{tabular}{|l|c|c|c|}
      \hline
        Quantité & Int. forte & \ Int. EM\  & Int. faible \\
      \hline
      \hline
        \small Énergie-impulsion & & & \\
        \small Moment cinétique total & & & \\
        \small Parité de charge $ \mathcal{C} $ & & & \\
        \small Parité d'espace $ \mathcal{P} $ & & & \\
        \small Renversement du temps $ \mathcal{T} $ & & & \\
        \small $ Q $ (charge électrique) & & & \\
        \small $ L_e $ (nb. électronique) & & & \\
        \small $ L_\mu $ (nb. muonique) & & & \\
        \small $ L_\tau $ (nb. tauonique) & & & \\
        \small B (nb. baryonique) & & & \\
        \small I (isospin fort) & & & \\
        \small I3 (isospin fort) & & & \\
        \small S (étrangeté) & & & \\
        \small C (charme) & & & \\
        \small B (bottom) & & & \\
        \small T (top) & & & \\
        \small Parité-$ \mathcal{G}\equiv\mathcal{C}e^{i\pi I_2} $ & & & \\
        \small $ \mathcal{CPT} $ & & & \\
      \hline
    \end{tabular}

  \end{center}
  \caption{Tableau récapitulatif des lois de conservations du modèle standard.}
\end{table}
...
\begin{table}[!htb]
  ...
  \end{center}
  \caption{Tableau récapitulatif des paramètres du modèle standard.}
\end{table}
...
\begin{table}[!htb]
  ...
  \end{center}
  \caption{Le lagrangien complet du modèle standard.}
\end{table}
...
Le modèle standard a été testé avec grande précision, néanmoins, la découverte du boson de Higgs n'a toujours pas eu lieue à cause de sa faible section efficace d'interaction. L'objectif de cette section est de déterminer théoriquement des limites inférieure et supérieur à la masse du boson de Higgs puis de faire l'état des lieu des limites expérimentales établies auprès d'expériences comme le Large Electron Positron collider (LEP) au Cern et le Tevatron au Fermilab.
...
\begin{figure}[!htb]
  ...
  \end{center}
  \caption{\label{higgs_autocouplage}Quelques diagrammes contribuant à l'autocouplage quadrilinéaire du boson de Higgs : zéro boucle (a), une boucle (b) et (c).}
\end{figure}
...
En l'absence de la découverte d'un boson de Higgs léger ($ M_\mathrm{Higgs}<1\TeV $), les signe d'une nouvelle physique devraient apparaitre à l'échelle du $ \TeV $. La figure \ref{prediction} montre les bornes supérieure et inférieure sur la masse du boson de Higgs en fonction l'échelle d'énergie $ \Lambda_\mathrm{cut} $.

\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=7cm, keepaspectratio]{phd_ms/prediction}
  \end{center}
  \caption{\label{prediction}Bornes théoriques supérieure et inférieure de la masse du boson de Higgs en fonction l'échelle d'énergie $ \Lambda $ obtenues à partir des arguments de trivialité et de stabilité du vide. La masse du quark top est $ m_t=175\GeV $, et $ \alpha_S=0.118 $.}
\end{figure}
...
où $ \lambda_t $ est le couplage de Yukawa du quark top.
...
La figure \ref{prediction} montre les bornes supérieure et inférieure sur la masse du boson de Higgs en fonction l'échelle d'énergie $ \Lambda_\mathrm{cut} $.

\subsection{Limites expérimentales}

\newpage

\section{Limites du modèle standard}
