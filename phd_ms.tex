...
Le mouvement d'une particule est identifié par son \emph{vecteur de position} (tenseur de rang un) noté $ \vec r $ et de coordonnées $ r^i $. La \emph{vitesse} d'une particule est définie par :
\begin{displaymath}
...
\begin{figure}[!ht]
  \centering
  \includegraphics[scale=0.2]{phd_ms/meca1}
\end{figure}
...
C'est les équations d'Euler-Lagrange, elles permettent de déterminer les équations du mouvement d'un système quelconque.

\newpage

Observons l'équation \ref{euler_lagrange}, elle contient $ n $ équations différentielles d'ordre 2 en $ q^i $, $ i=1\cdots n $. De telles équations s'intégrent et donnent des solutions uniques pour des conditions initiales $ q^i(t_0=0) $ et $ \dot q^i(t_0=0) $ connues.
...
\begin{eqnarray}
  S & = & \int_{t_1}^{t_2}dt\,\underbrace{L\left(q^i,\dot q^i\right)}_{\int d^3x\,\mathcal{L}\left(\phi(q^i,t),\partial_\mu\phi(q^i,t)\right)}\nonumber\\
    & = & \int d^4x\,\mathcal{L}\left(\phi(q^i,t),\partial_\mu\phi(q^i,t)\right)
\end{eqnarray}
...
C'est les équations de champ d'Euler-Lagrange, elles permettent de déterminer les équations de champ d'un système quelconque.
...
Nous verrons que les champs spineurs permettent de décrire les fermions, c'est-à-dire les particules qui constituent la matière et qui satisfont au principe d'exclusion de Pauly.
...
Il ont toutes les caractéristiques de projecteurs, en effet, il est immédiat de montrer que $ P^2_{L,R}=P_{L,R} $ et que $ P_L+P_R=\bbone_4 $. Pour un bispineur, ces opérateurs permettent de sélectionner les composantes de chiralité gauche ($ L $) pour $ P_L $ et droite ($ R $) pour $ P_R $. En possant:
\begin{equation}
...
La chromodynamique quantique fut proposée en 1973 par Hugh David Politzer, Frank Wilczek et David Gross pour tenter de comprendre la structure des hadrons (baryons et mesons). Les expériences ont montré qu'ils sont composés de \emph{quarks} et qu'ils sont soumis à l'interaction forte. Il est admit qu'il existe six saveurs de quarks répartis en deux familles. La première pour les quarks de charge électrique $ +\frac{2}{3}e $ et la seconde pour les quarks de charge électrique $ -\frac{1}{3}e $. De même que pour les leptons, les quarks sont classés en trois générations : $ \binom{u}{d} $, $ \binom{c}{s} $ et $ \binom{t}{b} $.
    ...
    \begin{fmffile}{qcd_graph1}
      \fmfframe(1,7)(1,7){ 
       \begin{fmfgraph*}(110,62)
        ...
        \fmflabel{$\bar q$}{i2}
        \fmflabel{$g$}{o1}
        \fmflabel{$-ig_s\frac{T^\alpha}{2}\gamma_\mu\ $}{v1}
       \end{fmfgraph*}
      }
    \end{fmffile}
...
\end{equation}
blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla blabla.

Les expériences ont permit de découvrir un certain nombre de propriétés propres à l'interaction faible : i) Pour les particules (resp. antiparticules) la composante droite (resp. gauche) n'a pas de charge faible. ii) Pour les particules (resp. antiparticules) la composante gauche (resp. droite) a une charge faible et interagit par paires de particules. Si on introduit la notion d'isospin faible $ I^\mathrm{W} $ dont la troisième composante $ I^\mathrm{W}_3 $ est interprété comme la charge faible, les particules peuvent être classées en doublets d'isospin faible :
\begin{equation}
...
Le modèle standard de Glashow, Salam et Weinberg (modèle GSW) est basé sur le groupe de jauge $ G $ formé du produit direct des groupes de jauge $ SU(2) $ pour l'isospin faible et $ U(1) $ pour l'hypercharge :
\begin{equation}
...
où $ \theta_{12} $, $ \theta_{12} $, $ \theta_{23} $ sont des angles de mélange, $ \delta $ une phase de violation CP, $ s_{ij}\equiv\sin\theta_{ij} $ et enfin $ c_{ij}\equiv\cos\theta_{ij} $. Les valeurs expérimentales des angles de mélange ont été déterminés expérimentalement et mènent à :
\begin{equation}
  M_\mathrm{CKM}=\left(
  \begin{array}{ccc}
...
Le premier modèle expliquant l'acquisition de masse par les particules fut proposé en 1964 parallèlement par Gerry Guralnik, Carl Richard Hagen, et Tom Kibble, Robert Brout et François Englert (figure \ref{photographie} a) ainsi que par Peter Higgs (figure \ref{photographie} b). Ce modèle est désormais connu sous le nom de \emph{mécanisme de Higgs}. C'est un pilier du modèle standard de la physique des particules, cependant, une des ses principales prédictions, le \emph{boson de Higgs}, n'a toujours pas été vérifiée expérimentalement.
...
Cette configuration brise la symétrie $ SU(2)_L $. Nous voyons ainsi apparaitre $ h(x) $ le champs scalaire associé au \emph{boson de Higgs}. Regardons maintenant comment sont générées les masses des bosons $ W^\pm $ et $ Z $ en calculant les termes de masses du lagrangien $ \mathcal{L}_\mathrm{Higgs} $ dans la limite où $ h(x) \rightarrow 0 $ :
\begin{equation}
...
La masse du boson de Higgs est un \underline{paramètre libre} du modèle standard de la physique de la physique des particules. Il reste maintenant à déterminer comment les fermions acquièrent une masse. Examinons le couplage de trilinéaire Yakawa entre le doublets des leptons et le champ $ \Phi_{0,L} $  dans la limite où $ h(x) \rightarrow 0 $ :
\begin{eqnarray}
...
où les coefficients $ \lambda_l $ et $ \lambda_q $ sont appelés les couplages de Yukawa. Au final, le modèle de Higgs permet d'expliquer l'origine des masses des bosons $ W^\pm $ et $ Z $ ainsi que celles de tous les fermions. Dans le modèle standard minimal de la physique des particules, les neutrinos ont une masse nulle. Des mesures expérimentales ont cependant montrée qu'ils ont une très faible de masse inférieure à l'électron volte.
...
Il est possible de construire deux type de termes de lagrangiens de masse, un premier de Dirac et un second de Majorana\footnote{A l'inverse des particules de Dirac, les particules de Majorana sont leurs propres antiparticules.} :
\begin{equation}
...
\begin{figure}[!ht]
  ...
  \end{center}
  \caption{\label{photographie}(a) Photographie de Tom Kibble, Gerry Guralnik, Carl Richard Hagen, François Englert et Robert Brout lors de la remise du Prix Sakurai 2010. (b) Photographie de Peter Higgs.}
\end{figure}

\subsection{Couplages du boson de Higgs}
...
D'un point de vue classique, la gravitation est décrite par la théorie de la relativité générale. Elle fut établie en 1925 par Albert Einstein et permet une description remarquable de l'univers et des objets astrophysiques. D'un point de vue quantique, il n'existe actuellement aucune théorie complètement satisfaisante même si de bons candidats existent, citons par exemple la supergravité, la théorie des cordes, la théorie des supercordes ou encore la gravitation à boucles. Il faut cependant noter que l'interactions gravitationnelle entre des particules fondamentales est extrêmement faible, au moins trente ordres de grandeur de moins que l'interaction faible. Dans la pratique, la gravitation est tout simplement ignorée.
...
\begin{table}[!ht]
  \begin{center}
    ...
    \begin{tabular}{|l|c|c|c|c|}
      ...
      Masse & 0 & 0 & & 0 \\
      \hline
      Portée & $ \infty $\footnotesize, confinée $ <10^{-15} $ m & $ \infty $ & $ <10^{-18} $ m & $ \infty $ \\
      \hline
      Source & couleur & charge électrique & charge faible & énergie/masse \\
      \hline
      Couplage & $ \alpha_S=0.118\pm0.003 $ & $ \alpha $ & & $ \sim4.6\times10^{-40} $ \\
      \hline
    \end{tabular}

  \end{center}
  \caption{Tableau récapitulatif interactions fondamentales.}
\end{table}
...
\begin{table}[!ht]
  ...
  \end{center}
  \caption{Tableau récapitulatif des paramètres du modèle standard (2011).}
\end{table}
...
\subsection{Limites expérimentales sur collisionneur}

\subsubsection{Limites indirectes}

Les expériences menées auprès du LEP, du SLC et du Tevatron ont permis de mesurer les paramètres du secteur électrofaible avec une précision suffisante pour tester le modèle standard au-delà du premier ordre des perturbations. La mesure indirecte de la masse du boson de Higgs est basée sur les corrections radiatives à la masse du boson $ Z $. Les diagrammes de Feynman de la figure \ref{higgs_correction} montrent les principales contributions. La première (figure \ref{higgs_correction} (a)) est proportionnelle à $ m^2_t $. Cette dépendance avait permis de prédire la masse du quark top en 1994 par Gerard't Hooft et Martinus Veltmanavant avant sa découverte par les expériences CDF en 1994 puis DØ en 1995. La seconde (figure \ref{higgs_correction} (b)) est proportionnelle à $ \ln M_\mathrm{Higgs} $.

La figure \ref{} (a) montre la comparaison entre la détermination indirecte de la masse du boson de Higgs (LEP1, SLD) et détermination directe de la masse du boson de Higgs (LEP2, Tevatron). Ces résultats tendent à favoriser une faible masse.
...
Parallèlement, un ajustement global des paramètres du secteur électrofaible a été réalisé. La figure \ref{} (b) montre la distribution $ \Delta\chi^2=\chi^2-\chi^2_\mathrm{min} $ en fonction de la masse du boson de Higgs. Elle impose les contraintes $ M_\mathrm{Higgs}=96^{+60}_{-38}\GeV $ et $ M_\mathrm{Higgs}<219\GeV $ à $ 95\% $ de niveau de confiance. Ces résultats tendent toujours à favoriser une faible masse.
...
\begin{figure}[!ht]
  ...
  \end{center}
  \caption{\label{}Blabla.}
\end{figure}

\subsubsection{Limites directes}

Le LEP a activement recherché le boson de Higgs. Les deux principaux canaux étudiés furent $ ee\to Z^*\to ZH $ (aussi appelé Higgs-strahlung) et $ H\to b\bar b $.

\paragraph{a) Higgs-strahlung}

L'étude du Higgs-strahlung (figure \ref{}) permet l'explorer les régions de très faible masse en mesurant l'énergie de recul qu'acquirent les boson en rayonnant un boson de Higgs. Durant la première phase de prise de donnée du LEP, aucun signal ne fut détecter dans aucune des quatre principales expériences Aleph, Delphi, Opal et L3. Durant la seconde phase de prise de donnée du LEP, avec une énergie dans le centre de masse atteignant $ 209\GeV $, le Higgs-strahlung fut le canal principal de production. La combinaison des résultats des quatre expériences permit d'établir la contrainte $ M_\mathrm{Higgs}>114.4\GeV $ à $ 95\% $ de niveau de confiance.
...
\paragraph{b) $ H\to b\bar b $}

\begin{figure}[!ht]
  \begin{center}
    \includegraphics[width=10cm, keepaspectratio]{phd_ms/aleph}
  \end{center}
  \caption{\label{}Blabla.}
\end{figure}
...
\paragraph{c) Résultats au Tevatron}

Plus récemment, les expériences DØ et CDF au Tevatron ont accumulé suffisamment de statistique afin de poser des limites en s'appuyant sur l'étude des canaux $ H\to WW $, $ H\to ZZ $, $ WH\to l\nu b\bar b $, $ WH/ZH\to qqb\bar b $, $ WH/ZH\to METb\bar b $, $ ZH\to llb\bar b $, $ H\to\gamma\gamma $, $ H\to\tau\tau $, $ H\to b\bar b $, etc...

\begin{figure}[!ht]
  \begin{center}
    \includegraphics[width=9cm, keepaspectratio]{phd_ms/exp3}
  \end{center}
  \caption{\label{}Blabla.}
\end{figure}
