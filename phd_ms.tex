...
\begin{itemize}
  ...
  \item $ \mu^2<0 $ $ \Rightarrow $ $ \left\{\begin{array}{l} \mathrm{un\ minimum\ trivial\ }\left<0\right|\Phi_L\left|0\right>=0 \\ \mathrm{un\ minimum\ non\ trivial\ }\boxed{\left<0\right|\Phi_L\left|0\right>=-\frac{\mu^2}{\lambda}\equiv\frac{v}{\sqrt{2}}} \end{array}\right. $
\end{itemize}
...
\begin{equation}
  \Phi'_L\rightarrow\Phi_L=\exp\left(i\frac{\sigma^a}{2}\chi_a(x)\right)\Phi'_L
\end{equation}
...
Cette configuration brise la symétrie $ SU(2)_L $. Nous voyons ainsi apparaitre $ h(x) $ le champs scalaire associé au \emph{boson de Higgs}. Regardons maintenant comment sont générées les masses des bosons $ W^\pm $ et $ Z $. Essayons de calculer les termes de masses du lagrangien $ \mathcal{L}_\mathrm{Higgs} $ dans la limite où $ h(x) \rightarrow 0 $ :
\begin{equation}
...
\end{equation}
\begin{equation}
  \boxed{M_W=\frac{vg}{2}\hspace{0.33cm}\mathrm{,}\hspace{0.33cm}M_Z=\frac{v\sqrt{g^2+g'^2}}{2}\hspace{0.33cm}\mathrm{et}\hspace{0.33cm}M_A=0}
\end{equation}
\begin{equation}
  \boxed{\frac{M_W}{M_Z}=\cos\theta_W<1}
\end{equation}
...
\begin{eqnarray}
  \mathcal{L}_\mathrm{Yukawa\,leptons} & = & \sum_{l=e,\mu,\tau}-G_l\left[\left(\bar\nu_l\ \ \bar l_L\right)\binom{0}{\frac{v}{\sqrt{2}}}l_R+\bar l_R\left(0\ \ \frac{v}{\sqrt{2}}\right)\binom{\nu_l}{l_L}\right]\nonumber\\
                              & = & \sum_{l=e,\mu,\tau}-G_l\frac{v}{\sqrt{2}}\left(\bar l_Ll_R+\bar l_R l_L\right)\nonumber
\end{eqnarray}

c'est-à-dire :
\begin{equation}
  \boxed{\mathcal{L}_\mathrm{Yukawa\,leptons}=\sum_{l=e,\mu,\tau}-G_l\frac{v}{\sqrt{2}}\bar ll}
\end{equation}

et en utilisant la matrice CMK (équation \ref{matrice_ckm}):
\begin{equation}
  \boxed{\mathcal{L}_\mathrm{Yukawa\,quarks}=\sum_{q=u,d,c,s,t,b}-G_q\frac{v}{\sqrt{2}}\bar qq}
\end{equation}

où les coefficients $ G $ sont appelés les couplages de Yukawa. Au final, le modèle de Higgs permet d'expliquer l'origine des masses des bosons $ W^\pm $ et $ Z $ ainsi que celles de tous les fermions. Dans le modèle standard minimal de la physique des particules, les neutrinos ont une masse nulle.
    ...
    \begin{fmffile}{higgs_graph1}
      \fmfframe(1,7)(1,7){ 
       \begin{fmfgraph*}(110,62)
        ...
        \fmflabel{$H$}{o1}
        \fmflabel{$H$}{o2}
        \fmflabel{$\ \propto\lambda=\frac{M^2_\mathrm{Higgs}}{2v^2}$}{v1}
       \end{fmfgraph*}
      }
    \end{fmffile}
...
Précédemment, nous avons montré que le couplage $ \lambda $ est positif. Lorsque $ \Lambda\to+\infty $, pour respecter la condition $ \lambda(\Lambda)>0 $, il faut imposer $ \lambda(\Lambda_0)\to0 $. La théorie devient alors triviale c'est-à-dire une théorie de champs libres. Pour résoudre ce paradoxe, il faut supposer l'existence d'une énergie limite $ \Lambda_{cut} $ au delà de laquelle le modèle standard n'est plus valide. Cette coupure fournit alors une limite supérieure au couplage $ \lambda $ mais aussi une limite supérieure à la masse de boson de Higgs :
\begin{equation}
  \boxed{M^2_\mathrm{Higgs}=2\lambda v<\frac{8\pi^2v^2}{3\ln\frac{\Lambda^2_{cut}}{v^2}}}
\end{equation}
...
En l'absence de la découverte d'un boson de Higgs léger ($ M_\mathrm{Higgs}<1\TeV $), les signe d'une nouvelle physique devraient apparaitre à l'échelle du $ \TeV $. La figure \ref{prediction} montre les bornes supérieure et inférieure sur la masse du boson de Higgs en fonction l'échelle d'énergie $ \Lambda $.

\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=7cm, keepaspectratio]{phd_ms/prediction}
  \end{center}
  \caption{\label{prediction}Bornes supérieure et inférieure sur la masse du boson de Higgs en fonction l'échelle d'énergie $ \Lambda $. La masse du quark est $ 175\GeV $, et $ \alpha_S=0.118 $.}
\end{figure}
