...
Les matrices de Dirac satisfont à l'algèbre de Clifford, en effet, l'anticommutateur de deux matrices donne : $ \left\{\gamma^\mu,\gamma^\nu\right\}=2g^{\mu\nu} $. Il est possible de réécrire les lagrangiens précédents en utilisant des bispineurs :
...
Il ont toutes les caractéristiques de projecteurs, en effet, il est immédiat de montrer que $ P^2_{L,R}=P_{L,R} $ et que $ P_L+P_R=\bbone_4 $. Ces opérateurs permettent de sélectionner les composantes de chiralité gauche ($ L $) pour $ P_L $ et droite ($ R $) pour $ P_R $ d'un bispineur. En possant:
\begin{equation}
...
\subsubsection{Invariance de jauge locales abéliennes}
...
\subsubsection{Invariance de jauge locales non-abéliennes}
...
La masse du boson de Higgs est un \underline{paramètre libre} du modèle standard de la physique de la physique des particules. Il reste maintenant à déterminer comment les fermions acquièrent une masse. Examinons le couplage de Yakawa entre le doublets des leptons et le champ $ \Phi_{0,L} $  dans la limite où $ h(x) \rightarrow 0 $ :
\begin{eqnarray}
...
où les coefficients $ \lambda_l $ et $ \lambda_q $ sont appelés les couplages de Yukawa. Au final, le modèle de Higgs permet d'expliquer l'origine des masses des bosons $ W^\pm $ et $ Z $ ainsi que celles de tous les fermions. Dans le modèle standard minimal de la physique des particules, les neutrinos ont une masse nulle. Des mesures expérimentales ont cependant montrée que les neutrinos ont une très faible de masse inférieure à l'électron volte.
