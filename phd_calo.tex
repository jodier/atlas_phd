...
Les signaux ce chaque cellules sont envoyés hors du cryostats dans les FEC (Front End Crate). Ceux-ci contiennent trois type de cartes électroniques :

\vspace{-0.25cm}

\paragraph{Les FEB (Front End Board)} Ces carte recueillent les signaux de 128 canaux. Elles effectuent la mise en forme des signaux et leur digitalisation en deux principales étapes : i) Les signaux sont pre-amplifiées avec un LNA (Low Noise Amplifier) et sont divisés en trois gains de rapports respectifs $ 1/9.9/93 $ afin de disposer d'une large dynamique. Comme la durée de vie d'un signal est supérieur à la fréquence de croisement des faisceaux du LHC, afin de limiter les effets de l'empilement, une mise en forme est effectuée par des filtres analogiques de type $ CR-(RC)^2 $. Le signal obtenu est d'air nul et sa largeur en temps est approximativement deux fois plus petite que la largeur en temps du signal triangulaire. ii) Les signaux mise en forme sont envoyés dans des tampons capacitifs, en attendant la décision du système de déclenchement L1CaloEM ($ <25\mu s $), avant d'être digitalisés à l'aide de convertisseurs analogique/numérique $ 12 $ bits\footnote{Le signal $ s $ est représenté par un nombre entier $ 12 $ bits c'est-à-dire $ 0\le s[ADC]\le\underbrace{2^{12}-1}_{4095} $.}. Afin d'économiser des ressources matérielles, le signal est échantillonné, une conversion toute les $ 25ns $ et seuls $ 5 $ échantillons, autour du maximum du signal, sur $ 32 $ sont conservés et envoyés à l'électronique \emph{back-end} à l'aide des transmetteurs optiques déjà cités au chapitre précédent, les OTX (Optical Transmiter).
...
\begin{figure}[!p]
  ...
  \end{bigcenter}
  \caption{Représentation en image de synthèse des FEC (Front End Crate).}
\end{figure}
...
\begin{figure}[!ht]
  ...
  \end{bigcenter}
  \caption{\label{}Photographie d'une FEB (Front End Board) (a) et photographie d'un ROD (Read Out Driver) (b).}
\end{figure}
...
\begin{figure}[!p]
  ...
  \end{bigcenter}
  \caption{\label{stabilite1}Distribution de la stabilité des piédestaux pour le calorimètre électromagnétique, gain haut (a). Distribution de la stabilité des piédestaux pour les calorimètres vers l'avant, gain moyen (b).}
\end{figure}
...
\begin{figure}[!p]
  ...
  \end{bigcenter}
  \caption{\label{stabilite2}Distribution de la stabilité des rampes pour les calorimètres vers l'avant, gain moyen (a). Distribution de la stabilité des rampes pour les calorimètres hadroniques bouchons, gain moyen, gain haut (b).}
\end{figure}
