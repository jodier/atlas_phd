...
Dans cette sous-section, nous allons étudier la possibilité d'utiliser des électrons que qualité lâche (\emph{loose}). Il faut savoir que pour les électrons \emph{loose}, il n'y a pas de critère d'association entre les traces et les amas c'est pourquoi nous allons utiliser conjointement la qualité lâches++ (\emph{loose++}) qui introduit une association traces-amas moins forte que la qualité intermédiaire (\emph{medium}) (voir chapitre \ref{chapitre_reco}). Au total, deux groupes de trois situations ont été analysées :
\begin{itemize}
